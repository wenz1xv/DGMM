...
from dock_comp import schrodinger_dock, vina_dock
...
class Filter:
    ...
    Method:
        run: run filter
    """
    def __init__(self, method,dock_grids, simthd, qedthd, sathd,sim_increase=0):
        self.dscore_map = {}
        self.method = method
        if self.method not in ['schrodinger', 'vina']:
    ...
    def batchdock(self, smiles, dock_grid, thread=0):
        if self.method == 'vina':
            ...
            scores = schrodinger_dock(smiles, dock_grid)
        return scores
...
class EDmodel:
    ...
    """
    def __init__(self, model_path, childsize, simthd, qedthd, sathd, ex_constraint, discrete, thread, maxtrys=80, MAXLEN=80):
        self.encoder = None
        self.decoder = None
    ...
    def load_model(self, model_path):
        try:
            self.encoder = load_model(f'{model_path}TVAE/DGMM_sfi_encoder.h5', custom_objects={'Sampling': Sampling})
            self.decoder = load_model(f'{model_path}TVAE/DGMM_sfi_decoder.h5')
            if self.discrete:
                self.latent2vec = load_model(f'{model_path}DVAE/latent2vec.h5', custom_objects={'VectorQuantizer':VectorQuantizer})
                self.vec2latent = load_model(f'{model_path}DVAE/vec2latent.h5')
                self.Qdecoder = load_model(f'{model_path}DVAE/Qdecoder.h5', custom_objects={'Sampling':Sampling})
    ...
    def encode(self, smiles):
        ...
        if self.discrete:
            qlatent, perplexity = self.latent2vec.predict(z, verbose=0)
            dgene = self.get_code_indices(qlatent)
            return dgene.numpy()
        else:
    ...
    def gene2pro(self, genes):
        if self.discrete:
            encodings = tf.one_hot(genes, self.codebook.shape[1])
            latents = tf.matmul(encodings, self.codebook, transpose_b=True)
            latents = tf.reshape(latents, [tf.shape(genes)[0],  self.codebook.shape[0], -1])
            latent, student_fp_pred, cgene_pred = self.Qdecoder(latents)
            pro_pred = latent.numpy() # if not converted,   pro_pred = tf.nn.softmax(latent).numpy()
        else:
...
class DGMMmodel:
    def __init__(self, model_path, popsize=100, genebatch=8, ex_constraint=None,
                 ...
                 path='result', compete=False, loglevel=0, discrete=False):
        self.MAXLEN = 80
        
        self.EDmodel = EDmodel(model_path, childsize, simthd, qedthd, sathd, ex_constraint, discrete, thread)
        self.Filter = Filter(method, dock_grids, simthd, qedthd, sathd)
    ...
    def mutation(self, _smi, npoint=0):
        ...
        if self.discrete:
            mol_gene = self.EDmodel.encode(smi)
            npoint = int(mol_gene.shape[1]*self.vfactor*0.1)
            for i in range(mol_gene.shape[0]):
                idx = np.random.choice(np.arange(mol_gene.shape[1]),np.random.randint(0, npoint), replace=False)
                mol_gene[i,idx] = np.random.choice(np.arange(np.max(mol_gene)), len(idx))
            return mol_gene
        else:
    ...
    def crossover(self, _smi1, _smi2, npoint=0):
        ...
        if self.discrete:
            mol_gene1 = self.EDmodel.encode(smi1)
            mol_gene2 = self.EDmodel.encode(smi2)
            npoint = int(mol_gene1.shape[1]*self.vfactor*0.1)
            for i in range(mol_gene1.shape[0]):
                idx = np.random.choice(np.arange(mol_gene1.shape[1]),np.random.randint(0, npoint), replace=False)
                mol_gene1[i,idx] = mol_gene2[i, idx]
            return mol_gene1
        else:
